...
describe("PATCH /comments/:commentId/replies/:replyId/votes/downvote", () => {
  beforeEach(async () => {
  ...
  it("Should add a downvote (new vote) to a reply (toggle)", async () => {
    const url = `/v1/comments/${oldComment.commentId}/replies/${oldReply.replyId}/votes/downvote`;
    ...
    expect(addRes.body.data).toEqual({
      ...
      numOfUpVotes: oldReply.numOfUpVotes,
      numOfDownVotes: oldReply.numOfDownVotes + 1,
    });
    ...
    await ReplyModel.findById(oldReply.replyId).then((reply) => {
      expect(reply).toBeTruthy();
      expect(reply.upVotes.length).toEqual(oldReply.numOfUpVotes);
      expect(reply.downVotes.length).toEqual(oldReply.numOfDownVotes + 1);
      expect(reply.downVotes.length + reply.upVotes.length).toEqual(
  ...
  it("Should add a downvote (upvote exists) to a reply (toggle)", async () => {
    const url = `/v1/comments/${oldComment.commentId}/replies/${oldReplyWithUpVote.replyId}/votes/downvote`;
    const bearerToken = `bearer ${global.appToken}`;
    const ownerIdUpdate = "voter@gmail.com";
    ...
    expect(addRes.body.data).toEqual({
      ...
      numOfVotes: oldReplyWithUpVote.numOfVotes,
      numOfUpVotes: oldReplyWithUpVote.numOfUpVotes - 1,
      numOfDownVotes: oldReplyWithUpVote.numOfDownVotes + 1,
    });
    ...
    await ReplyModel.findById(oldReplyWithUpVote.replyId).then((reply) => {
      ...
      expect(reply.downVotes.length).toEqual(
        oldReplyWithUpVote.numOfDownVotes + 1
      );
      expect(reply.downVotes.length + reply.upVotes.length).toEqual(
        oldReplyWithUpVote.numOfVotes
      );
  ...
  it("Should remove a downvote from a comment (toggle)", async () => {
    const url = `/v1/comments/${oldComment.commentId}/replies/${oldReplyWithDownVote.replyId}/votes/downvote`;
    const bearerToken = `bearer ${global.appToken}`;
    const ownerIdUpdate = "voter@gmail.com";
    ...
    expect(removeRes.body.data).toEqual({
      ...
      numOfVotes: oldReplyWithDownVote.numOfVotes - 1,
      numOfUpVotes: oldReplyWithDownVote.numOfUpVotes,
    ...
    await ReplyModel.findById(oldReplyWithDownVote.replyId).then((reply) => {
      ...
      expect(reply.downVotes.length + reply.upVotes.length).toEqual(
        oldReplyWithDownVote.numOfVotes - 1
      );
    });
  });
