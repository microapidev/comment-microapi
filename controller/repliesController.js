...
try {
    ...
    const parentComment = await Comments.findById({ _id: commentId
    });
    ...
    if (!reply) {
      return next(
        ...
        if (comment.downVotes.includes(ownerId)) {
          ...
          const voterIndex = comment.reply.downVotes.indexOf(ownerId);
          //if index exists
          if (voterIndex > -1) {
            //delete that index
            comment.reply.downVotes.splice(voterIndex, 1);
          }
        }

        //same as above for upvotes
        if (comment.reply.upVotes.includes(ownerId)) {
          const voterIdx = comment.reply.upVotes.indexOf(ownerId);
          if (voterIdx > -1) {
            comment.reply.upVotes.splice(voterIdx, 1);
          }
        } else {
          // add user to the top of the upvotes array
          comment.reply.upVotes.unshift(ownerId);
        }
        ...
        const totalUpVotes = comment.reply.upVotes.length;
        const totalDownVotes = comment.reply.downVotes.length;
        ...
        return responseHandler(res, 200, data, "Reply upVoted Successfully!");
    } 
catch (error) {
    return next(
      new CustomError(500, "Something went wrong, please try again later", error)
